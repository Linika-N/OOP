...
public class MiddleVendingMashine extends VendingMashine {
    /**
     * 
    ...
    public MiddleVendingMashine(String name, String productsCodes) {
        super(name, productsCodes);
        if (!checkCodes(productsCodes))
            throw new IllegalArgumentException("Нельзя создать данный тип автомата - Интервал кодов может быть от 1 до 50");
    ...
    public boolean checkCodes(String interval) {
        ...
        if (toCheck.length > 2)
            return false;
        else if(Integer.parseInt(toCheck[0])<1||Integer.parseInt(toCheck[1])<50) 
            return false;
        else {
    ...
    public void addProduct(int code, Product product, int productCode) {
        if (productCode < typeProductFrom || productCode > typeProductTo)
            throw new IllegalArgumentException("Код товара не подходит для данного типа автомата. Коды товаров для автомата - от 1 до 50");
        else
